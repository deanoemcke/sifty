 ...
 *
 * Idempotent: only rows with embedding IS NULL are processed, and each chunk is
 * persisted immediately after it's embedded, so a re-run after a partial failure
 * (network error, rate limit, etc.) resumes without re-spending API calls on
 * already-embedded rows.
 */

import Database from 'better-sqlite3';
import { DB_PATH } from '../src/server/db';
import { embedTextsBatchAsync } from '../src/server/embeddings';
...
async function run(): Promise<void> {
  ...
  const pending = db
    .prepare<
      [],
      PendingRow
    >('SELECT slug, display FROM trademe_categories WHERE embedding IS NULL')
    .all();
  ...
  const update = db.prepare('UPDATE trademe_categories SET embedding = ? WHERE slug = ?');
  let embeddedCount = 0;
  ...
  for (const chunk of chunkItems(pending, CHUNK_SIZE)) {
    ...
    const persistChunk = db.transaction(() => {
      chunk.forEach((row, index) => {
        update.run(JSON.stringify(embeddings[index]), row.slug);
      });
    });
    persistChunk();
