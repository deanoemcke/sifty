...
export async function handleDiscover(
  ...
  try {
    ...
    console.log(
      `[discover] step1 raw=${JSON.stringify(rawCategories)} → mapped slugs: ${chosenTop2.join(", ")}`,
    );
    if (chosenTop2.length === 0) {
      sendJSON(response, 500, { error: "AI returned no valid broad categories" });
      return;
    }
    if (chosenTop2.length < rawCategories.length) {
      sendJSON(response, 500, { error: "AI hallucination detected — please try again" });
      return;
    }
    ...
    const urls = collapsedEntries.map((entry) =>
      buildTrademeUrl(entry, discMaxPrice, discFulfillment, discRegionValue),
    );
    if (urls.length === 0) {
      sendJSON(response, 500, { error: "AI returned no valid specific categories" });
      return;
    }
    ...
    console.log(
      `[discover] "${discPrompt}" → step1: ${chosenTop2.join(", ")} → ${urls.length} URL(s) (incl. Facebook), name="${discName}"`,
    );
