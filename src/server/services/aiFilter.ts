// AI-filter domain service — batches listings, calls the LLM per batch, and
// parses/validates the response. No HTTP types: reused by both the SSE route
// (routes/aiFilter.ts) and the headless scheduler (scheduler.ts).
...
// Shared core reused by both the SSE route (routes/aiFilter.ts) and the
// headless scheduler (src/server/scheduler.ts): batches listings, calls the
// LLM per batch, and parses/validates the response — everything except how a
// caller wants to surface progress (SSE per batch vs. just the final array).
